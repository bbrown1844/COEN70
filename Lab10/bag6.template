...
namespace scu_coen70_10
{
    ...
    // of the target were removed.
    {
        ...
        {   // Target was found and there is no left subtree, so we can
            // remove this node, making the right child be the new root.
            oldroot_ptr = root_ptr;  //save the node for deletion
            root_ptr = root_ptr->right( );
            delete oldroot_ptr;
            return 1;
        }
    ...
    // Header file used: bintree.h
    {
        ...
        if (root_ptr == NULL)
        {
            //no bst tree so create a new node as the root_ptr
    ...
    typename bag<Item>::size_type bag<Item>::count(const Item& target) const
    {
        ...
        while(cursor!=NULL){
          ...
          //the nodes data field is equal to entry to increment answer
          //and keep searching for duplicates
          {
              ++answer;
              cursor = cursor->left();
          }
    ...
    // Header file used: bintree.h
    {
        ...
        //clears the bag that the operator is called on
        tree_clear(root_ptr);
        //copies over source
